...
- **Principle:** Define the minimum number of Pods that must remain available during voluntary disruptions (e.g., node maintenance, scaling).
- **Guidance for Copilot:**
    - Use PDBs to ensure application availability during planned disruptions.
- **Configuration:** Set `minAvailable` or `maxUnavailable` to control disruption tolerance.    
### **4. PodAntiAffinity and PodAffinity**
- **Principle:** Control Pod placement based on labels to improve availability and reduce risk of simultaneous failures.
- **Guidance for Copilot:**
    - Use `podAntiAffinity` to prevent Pods from being scheduled on the same node (e.g., for high availability).
    - Use `podAffinity` to co-locate Pods that need to communicate frequently.
...
```
### **5. PodSpreadConstraints**
- **Principle:** Define how Pods should be distributed across failure domains (e.g., nodes, zones) to improve resilience.
- **Guidance for Copilot:**
    - Use `podSpreadConstraints` to ensure Pods are evenly distributed across nodes or zones.
- **Configuration:** Specify `maxSkew`, `topologyKey`, and `whenUnsatisfied` to control distribution.
- **Example (PodSpreadConstraints):**
```yaml
spec:
  podSpreadConstraints:
    - maxSkew: 1
      topologyKey: "kubernetes.io/hostname"
...
```
### **6 Priority Classes**
- **Principle:** Assign priority to Pods to influence scheduling decisions, especially during resource contention.
- **Guidance for Copilot:**
    - Use Priority Classes to ensure critical applications are scheduled first.
- **Configuration:** Define `value`, `globalDefault`, and `description` for the priority
